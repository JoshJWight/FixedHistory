...
static bool pointInsideBox(point_t point, point_t boxCenter, float boxLenX, float boxLenY)
{
    return abs(point.x - boxCenter.x) < boxLenX / 2.0f && abs(point.y - boxCenter.y) < boxLenY / 2.0f;
}

static bool pointInsideCircle(point_t point, point_t circleCenter, float circleRadius)
{
    return math_util::dist(point, circleCenter) < circleRadius;
}

static bool verticalLineCircle(point_t bottom, point_t top, point_t circleCenter, float circleRadius)
{
...
static bool horizontalLineCircle(point_t left, point_t right, point_t circleCenter, float circleRadius)
{
...
static bool boxCircle(point_t boxCenter, point_t boxSize, point_t circleCenter, float circleRadius)
{
    ...
    else
    {
        ...
        point_t bottomRight = boxCenter + point_t(halfBox.x, -1.0f * halfBox.y);
